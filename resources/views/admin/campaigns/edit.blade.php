    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.campaigns.update", [$campaign->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dedup') ? 'is-invalid' : '' }}" type="number" name="dedup" id="dedup" value="{{ old('dedup', $campaign->dedup) }}" step="1" required>
                @if($errors->has('dedup'))
                    <div class="invalid-feedback">
                        {{ $errors->first('dedup') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('dedup_limit') ? 'is-invalid' : '' }}" name="dedup_limit" id="dedup_limit" required>
                    ...
                    @foreach(App\Models\Campaign::DEDUP_LIMIT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('dedup_limit', $campaign->dedup_limit) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('numbers') ? 'is-invalid' : '' }}" name="numbers[]" id="numbers" multiple>
                    @foreach($numbers as $id => $number)
                        <option value="{{ $id }}" {{ (in_array($id, old('numbers', [])) || $campaign->numbers->contains($id)) ? 'selected' : '' }}>{{ $number }}</option>
                    @endforeach
                </select>
